...
namespace BaristaJS.Appengine.Bundles.Npm.NodeNpm
{
    ...
    public class NpmSerialize : INpmSerialize
    {
        ...
        /// </summary>
        private static Regex s_searchRegex;

        /// <summary>
        /// Regular expression class for parsing search output
        /// </summary>
        private static Regex s_concatJsonRegex;
        ...
        public static string ConvertDependToPath(string depends)
        {
            ...
            var seps = new[] { '/' };
            var dependencies = depends.Split(seps);
            var relative = ".";
        ...
        public static string ConvertPathToDepends(string relativePath)
        {
            ...
            var moduleSeparator = new [] { ModuleSeparator };
            var modules = relativePath.Split(moduleSeparator, StringSplitOptions.RemoveEmptyEntries);
        ...
        public IEnumerable<INpmInstalledPackage> FromListInstalled(string listJson)
        {
            ...
            try
            {
                listObj = JsonConvert.DeserializeObject<Dictionary<string, object>>(listJson);
            }
            ...
            try
            {
                if (listObj != null && listObj.Count > 0)
                {
                    ...
                    InstalledPackageFromDictionary(installed, name as string, string.Empty, listObj);
                }
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<INpmInstalledPackage> FromListInstalledChildren(string listJson)
        {
            ...
            try
            {
                if (listObj != null && listObj.Count > 0)
                {
                    ...
                    InstalledDependenciesFromDictionary(installed, name as string, listObj);
                }
            }
            catch (Exception ex)
            {
        ...
        public INpmInstalledPackage FromListMatchInstalled(string listJson, INpmPackage package)
        {
            ...
            try
            {
                if (listObj != null && listObj.Count > 0)
                {
                    object name;
                    listObj.TryGetValue("name", out name);
                    matched = MatchPackageFromDictionary(package, name as string, string.Empty, listObj);
                }
            }
            catch (Exception ex)
            {
        ...
        public INpmRemotePackage FromView(string viewJson)
        {
            ...
            Dictionary<string, object> viewObj;

            try
            {
                viewObj = JsonConvert.DeserializeObject<Dictionary<string, object>>(viewJson);
            }
            catch (InvalidOperationException ex)
            {
        ...
        public IEnumerable<INpmPackageDependency> FromOutdatedDependency(string outdated)
        {
            ...
            var depends = new List<INpmPackageDependency>();
            var seps = new [] { '\n' };
            var lines = outdated.Trim().Split(seps, StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (var line in lines)
            {
                ...
                var dependency = new NpmPackageDependency();
                seps = new[] { '@' };
                var nameVersion = tokens[0].Split(seps);
                ...
                {
                    seps = new [] { '=' };
                    var current = tokens[2].Split(seps);
        ...
        {
            var results = new List<INpmSearchResultPackage>();
            if (output == null)
            {
                return results;
            }
            ...
            if (s_searchRegex == null)
            {
                s_searchRegex = new Regex(SearchPattern, RegexOptions.Compiled);
            }
            ...
            for (var ix = 1; ix < count; ix++)
            {
                ...
                var m = s_searchRegex.Match(lines[ix]);
                if (m.Success && m.Groups.Count > 1)
                {
                    ...
                    if (m.Groups.Count > 5)
                    {
                        var keywordlist = m.Groups[5].ToString();
                        keywords = keywordlist.Split(new [] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                    }

                    var result = new NpmSearchResultPackage(
                                                                            name,
                                                                            null,
                                                                            desc,
                                                                            author,
        ...
        {
            var installs = new List<INpmInstalledPackage>();

            if (s_concatJsonRegex == null)
            {
                s_concatJsonRegex = new Regex(ConcatJsonPattern, RegexOptions.Compiled);
            }

            // In case we get multiple Json object returned, split into multiple strings
            var matches = s_concatJsonRegex.Matches(output);
            var jsonStrings = new string[matches.Count + 1];

            var startIndex = 0;
            var stringNum = 0;
            foreach (Match match in matches)
            {
                var nextIndex = match.Index + 3;
            ...
            {
                Dictionary<string, object> installObj;
                try
                {
                    installObj = JsonConvert.DeserializeObject<Dictionary<string, object>>(jsonString);
                }
                catch (InvalidOperationException ex)
                {
                ...
                try
                {
                    ...
                    {
                        foreach (var module in installObj)
                        {
                            InstalledPackagesFromInstalledDictionary(installs, module.Value);
                        }
                    }
                }
                catch (Exception ex)
        ...
        public NpmException ExceptionFromError(string output)
        {
            ...
            var exception = new NpmException(NpmReportedError);
            ...
            foreach (var line in errorLines)
            {
                ...
                {
                    var errorText = line.Substring(NpmErrorLineStart.Length);
        ...
        protected static void FillPackageFromNameVersion(INpmPackage package, string nameVersion)
        {
            ...
            {
                var seps = new [] { '@' };
                var nameAndVersion = nameVersion.Split(seps);
        ...
        {
            var remote = new NpmRemotePackage
            {
                Name = name
            };
            ...
            {
                var dist = viewObj["dist-tags"] as Dictionary<string, object>;
                if (dist != null)
                {
        ...
        protected static NpmPackageDependency ConvertKeyValueToDependency(string key, object value)
        {
            var dependency = new NpmPackageDependency
            {
                Name = key, VersionRange = value as string
            };
            return dependency;
        }
        ...
        {
            var deps = obj as Dictionary<string, object>;
            if (deps != null)
            {
                var dependencies = new List<NpmPackageDependency>();
                foreach (var pair in deps)
                {
                    var dependency = ConvertKeyValueToDependency(pair.Key, pair.Value);
        ...
        protected static void InstalledPackagesFromInstalledDictionary(
                                            ...
                                            object child)
        {
            var childObj = child as Dictionary<string, object>;
            if (childObj == null)
            {
                return;
            }

            var installed = new NpmInstalledPackage();
            if (childObj.ContainsKey("what"))
            {
                FillPackageFromNameVersion(installed, childObj["what"] as string);
            }

            if (childObj.ContainsKey("parentDir"))
            {
                var parentDirectory = childObj["parentDir"] as string;
                if (parentDirectory != null)
                {
                    installed.DependentPath = ConvertPathToDepends(parentDirectory);
                }
            }
            else
            {
            ...
            {
                var array = childObj["children"] as ArrayList;
                if (array != null && array.Count > 0)
                {
                    ...
                    foreach (var item in array)
                    {
                        InstalledPackagesFromInstalledDictionary(parent, item);
                    }
                }
            }
                                                    ...
                                                    Dictionary<string, object> listObj)
        {
            ...
            if (listObj.ContainsKey("dependencies"))
            {
                ...
                {
                    var parentName = string.IsNullOrWhiteSpace(name) ? "." : name;
                    var mypath = string.IsNullOrWhiteSpace(dependentPath) ?
                                                        parentName :
                                                        dependentPath + "/" + parentName;
                    foreach (var pair in dependDict)
                    {
                        var val = pair.Value as Dictionary<string, object>;
                        if (val != null)
                        {
                            InstalledPackageFromDictionary(parent, pair.Key, mypath, val);
                        }
                    }
                }
                else
            ...
            if (listObj.ContainsKey("dependencies"))
            {
                ...
                if (dependDict != null && dependDict.Count > 0)
                {
                    ...
                    foreach (var pair in dependDict)
                    {
                        ...
                        if (val != null)
                        {
                            var installed = MatchPackageFromDictionary(package, pair.Key, mypath, val);
                            if (installed != null)
                            {
                                return installed;
                            }
