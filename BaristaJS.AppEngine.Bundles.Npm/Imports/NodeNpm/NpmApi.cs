...
namespace BaristaJS.Appengine.Bundles.Npm.NodeNpm
{
    ...
    public class NpmApi : INpmApi
    {
        ...
        /// </summary>
        private string m_rootWorkingDirectory;
        ...
        public void SetDependencyDirectory(string dependency)
        {
            if (dependency == null)
            {
                Client.WorkingDirectory = m_rootWorkingDirectory;
            }
            else
            {
                var path = ConvertDependToPath("./" + dependency);
                Client.WorkingDirectory = path;
            }
        }
        ...
        public string GetInstalledVersion()
        {
            ...
            Client.Execute("--version", null);
            var output = Client.LastExecuteOutput;
            return output.Trim();
        }
        ...
        {
            var rc = Client.Execute("list", "--json");
            if (rc == 0)
            {
                string output = Client.LastExecuteOutput;
                return Serializer.FromListInstalled(output);
            }
        ...
        private void Initialize(NpmFactory factory, string wd, Uri registry)
        {
            ...
            if (!string.IsNullOrWhiteSpace(wd))
            {
                ...
                this.m_rootWorkingDirectory = wd;
            }
        }
        ...
        {
            var relative = NpmSerialize.ConvertDependToPath(depends);
            if (relative != null)
            {
                var absolute = Path.Combine(this.m_rootWorkingDirectory, relative.Replace('/', '\\'));

                return absolute;
            }

            return m_rootWorkingDirectory;
        }
    }
}
